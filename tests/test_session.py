...
class TestDbAnalytics:
    ...
    def test_workshop_flag_no_longer_bypasses_consent(self, tmp_db):
        """The retired workshop flow let participants be recorded without consenting.

        The column is kept so past workshop sessions stay identifiable, but it must
        no longer act as a substitute for consent: analytics require consent outright.
        """
        import sqlite3

        sid = "test-legacy-workshop-sid"
        db_module.upsert_session(sid)
        conn = sqlite3.connect(str(tmp_db))
        conn.execute("UPDATE sessions SET is_workshop = 1 WHERE id = ?", (sid,))
        conn.commit()
        conn.close()
        ...
        conn = sqlite3.connect(str(tmp_db))
        assert conn.execute("SELECT view FROM render_stats").fetchone() is None
        assert conn.execute("SELECT event_type FROM page_events").fetchone() is None
        conn.close()
